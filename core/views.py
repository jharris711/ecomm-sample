        ...
        try:
            ...
            if form.is_valid():
                ...
                if use_default_shipping:
                    ...
                    if is_valid_form([shipping_address1, shipping_country, shipping_city, shipping_zipcode]):
                ...
                if same_billing_address:
                    ...
                    if is_valid_form([billing_address1, billing_country, billing_city, billing_zipcode]):
            ...
            messages.warning(self.request, "You don't have anything in your cart.")
        ...
        if form.is_valid():
            ...
            if save:
                if userprofile.stripe_customer_id != '' and userprofile.stripe_customer_id is not None:
                    customer = stripe.Customer.retrieve(userprofile.stripe_customer.id)
            ...
            if amount > 0:
                try:
                    ...
                    # yourself an email
                    messages.warning(self.request, "Something went wrong. Unable to charge card. Please try again.")
                    return redirect("/")
        ...
        if form.is_valid():
            try:
                ...
                if total >= 100:
                    ...
                    order.save()
                    messages.success(self.request, "Promo code applied to order")
                    return redirect("core:checkout")
                else:
                    messages.warning(self.request, f"Order must be ${limit} or more to apply promo code.")
                    return redirect("core:checkout")
            except ObjectDoesNotExist:
                messages.info(self.request, "You don't have an active order")
                return redirect("core:checkout")
    ...
    def post(self, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            try:
                ...
                refund = Refund()
                refund.order = order
                refund.reason_for_return = reason_for_return
                refund.email = email
                refund.save()
